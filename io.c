...
int cmd_prompt()
{
    ...
    */
    ...
    else if (!strcmp(token, "self"))
    {
        competition2();
    }
    else if (!strcmp(token,"baka"))
    {
        printf("waruguchihakakanaidene\n");
    }
...
unsigned int is_legalmove(const char *str, int dice)
{
    /* return = MOVE_NULL : illegal
              others    : a move ( validity is not guaranteed ) */
    ...
    {
        str_CSA_move(str_move, moves[i]);
        if (!strcmp(str_move, str))
            return moves[i];
    }
    return MOVE_NULL;
}
...
void out(const char *format, ...)
{
    ...
    if ((strchr(format, '\n') != NULL || strchr(format, '\r') == NULL) && file_log != NULL)
    {
        va_start(arg, format);
...
static void competition(){
    ...
    int count=0;
        ...
        //探索(TURNは0or1)
            if(TURN){
                ret=search_root2(dice);
            }else{
                ret=search_root(dice);
            }
            count++;
            if(ret==-3){
                //Stalemate
...
static void play(int k) {
    ...
    while (num == 0) {
        dice = get_dice();
        history_dice(dice);
        num = (g ^ TURN) ? search_root2(dice) : search_root(dice);
        if (num == -2){
            (g ^ TURN) ? WIN++ : LOSE++;
        }
        if (num == -3){
    ...
    out(" 戦績: プログラム1: %d 勝, プログラム2: %d 勝, 引き分け: %d 回 (%.3f)\n",WIN, LOSE, TIE, (double)WIN / (WIN + LOSE));
    game_initialize();
}
static void competition2() {
    int ngame;
